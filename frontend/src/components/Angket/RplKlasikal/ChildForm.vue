<template>
    <li>
        ...
        <ul v-if="dynamic_form.child.length > 0" class="border-left">
            <child-form v-for="(child, key) in dynamic_form.child" :dynamic_form="child" :my_key="key" :level="level + 1"
                :key="`child_${key}`" @emit_remove_from_child="reactEmitRemove" @emit_edit_from_child="reactEmitEdit"
                @emit_add_child_from_child="reactEmitAddChild"></child-form>
        </ul>
    </li>
</template>
<script>
