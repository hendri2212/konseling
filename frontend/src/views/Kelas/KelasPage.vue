        ...
        <CCard accent-color="primary">
            ...
            <CCardBody>
                <CDataTable :responsive="false" v-if="datatable" :items="items" :fields="fields" hover>
                    ...
                    <!-- <template #number_of_students="{ item }">
                        <td>{{ item.number_of_students }}</td>
                    </template> -->
                    <template #actions="{ item, index }">
                        <td>
                            <CDropdown color="none" toggler-text="Dropdown Button">
                                ...
                                <CDropdownItem
                                    @click="$router.push({ name: 'AssignSiswa', query: { kelas: item.id } })">
                    ...
                    <template #under-table>
                        <CPagination :activePage.sync="pagination.active" @update:activePage="getData"
                            :pages="pagination.max_page" align="center" />
                    </template>
                </CDataTable>
...
const fields = [
    ...
    // { key: 'aksi', tdClass: 'align-middle text-center', thClass: 'align-middle text-center' },
...
<style scoped>
    :deep(.table td, .table th) { vertical-align: middle !important; }
    :deep(.table td .dropdown-toggle), :deep(.table td .btn) { padding: 0 !important; }
    :deep(.table td .dropdown-toggle svg), :deep(.table td .btn svg) { margin: 0 !important; }
</style>